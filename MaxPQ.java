    ...
    **********************************************************************/
    ...
    private void sink(int k) {
        while (2*k <= N) {
            int j = 2*k;
            if (j < N && less(j, j+1)) j++; 
            // the reason we have to swap with the larger of the two child nodes is 
            // when the child node is swapped and becomes the parent it needs to be 
            // larger than the two children left
            if (!less(k, j)) break;
            exch(k, j);
            k = j;
        }
